...
// These 4 pages should bestruct Page *pp = NULL;
//
// Returns NULL if out of free memory.
//
// Hint: use page2kva and memset
...
page_alloc_4pages(int alloc_flags)
{
	...
	if (page_free_list) {
		...
		while (result->pp_link) {
			if (page2pa(result->pp_link) == page2pa(result) + PGSIZE
			    && page2pa(result->pp_link->pp_link) == page2pa(result->pp_link) + PGSIZE
			    && page2pa(result->pp_link->pp_link->pp_link) == page2pa(result->pp_link->pp_link) + PGSIZE) {
				found = 1;
				break;
			}
		...
		if (found) {
			...
			return NULL;
			// panic("page_alloc_4pages: out of memory");
		}
	} else {
		return NULL;
	}
...
{
	// Fill this function
	if (pp && pp->pp_link && pp->pp_link->pp_link
	    && pp->pp_link->pp_link->pp_link) {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  do {
    ...
    if ((a + PGSIZE > ULIM) || (*pte & perm) != perm) {
      user_mem_check_addr = (i == 0) ? (uint32_t)va : a;
      return -E_FAULT;
    }
  } while (++i < n);
